...
site = 'https://www.bing.com/maps?q=painting+contractors+new+york&form=QBLH&sp=-1&ghc=1&pq=painting+contractors+new+york&sc=5-29&qs=n&sk=&cvid=4CFF673BB6C34630A12E6E8EA288F578'
...
def scrapePage(city, site, page, typeOfContractor, notFinished=True, iteration=1, count=0):
	...
	while notFinished:
		...
		for i in range(len(search)):
			#company name
			soup2 = search[i].find_all("div", {"class": "b_factrow"})
			compName = soup2[0].get_text()
			#phone number
			phoneNumber = soup2[-1].get_text()
			#URL
			try:
				URL = str(search[i]).split('"website":"')[1].split(',')[0]
			except:
				print("")
...
def doALL(typeOfContractor, page):
	...
	for i in range(len(listOfCities)):
		...
		if len(citySplit) > 1:
			...
			city = citySplit[0]
		#so far have done lanscaping, plumber, bathroom, handyman, roofing+gutters, waste+material+removal, fence, concrete, tree+service
		site = 'https://www.bing.com/maps?q=' + typeOfContractor + '+contractors+' + city + '&form=QBLH&sp=-1&ghc=1&pq=painting+contractors+new+york&sc=5-29&qs=n&sk=&cvid=4CFF673BB6C34630A12E6E8EA288F578'
...
#CONTRACTING TYPES: lanscaping, plumber, bathroom, handyman, roofing+gutters, waste+material+removal, fence, concrete, tree+service
#flooring, pest control, maid+service, siding, garage+door,house+carpet, house+remodel, house+maid, carpenter, window, house+drywall
